...
public class EmployeeResource {
    ...
    /**
     * {@code check Employee   /employees/:id} : move the Employee to checking state.
     *
     * @param id the id of the employee to update.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @PutMapping("/employees/{id}")
    public ResponseEntity<String> checking(@PathVariable Long id) {
        log.debug("REST request to move Employee to check state : {}", id);
        StateMachine<HiringStatus, HiringEvents> result =  employeeService.checking(id);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, id+""))
            .body(result.getState().getId().name());
        
    }
    
    
    /**
     * {@code check Employee   /employees/onboarding/:id} : move the Employee to Approved state.
     *
     * @param id the id of the employee to update.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @PutMapping("/employees/onboarding/{id}")
    public ResponseEntity<String> approveEmployeeHiring(@PathVariable Long id) {
        log.debug("REST request to move Employee to approved state : {}", id);
        StateMachine<HiringStatus, HiringEvents> result =  employeeService.approveEmployeeHiring(id);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, id+""))
            .body(result.getState().getId().name());
    ...
    /**
     * {@code check Employee   /employees/rejecting/:id} : move the Employee to Rejected state.
     *
     * @param id the id of the employee to update.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @PutMapping("/employees/rejecting/{id}")
    public ResponseEntity<String> rejectEmployeeHiring(@PathVariable Long id) {
        log.debug("REST request to move Employee to rejected state : {}", id);
        StateMachine<HiringStatus, HiringEvents> result =  employeeService.rejectEmployeeHiring(id);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, id+""))
            .body(result.getState().getId().name());
